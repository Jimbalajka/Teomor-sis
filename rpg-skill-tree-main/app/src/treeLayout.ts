...
function relaxLayout(source: SkillNode[], minDist = 160, iterations = 12): SkillNode[] {
  const out = source.map((n) => ({ ...n }));
  // Двигаем почти всё; якоря — только центр и слоты Черт/Ремёсел
...
export function applyHighwayLayout(source: SkillNode[]): SkillNode[] {
  ...
  // Слоты Черт/Ремёсел ставим в конце — за крайним |x| дерева
  ...
  // Слоты — отдельные колонны за пределами веток
  let maxAbsX = 0;
  for (const p of pos.values()) maxAbsX = Math.max(maxAbsX, Math.abs(p.x));
  const slotX = Math.max(2400, maxAbsX + 420);
  ...
  // Лёгкий снап — визуальная сетка без ломки шоссе
